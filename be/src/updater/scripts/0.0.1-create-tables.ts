...
const ADD_FK_CONSTRAINT = async () => {
   await doInDbConnection(async (conn: Connection) => {
      ...
      await conn.query(`ALTER TABLE TBL_DATA_IMPORT_FILE ADD CONSTRAINT \`fk_tbl_data_import_file-1\` FOREIGN KEY IF NOT EXISTS (DATA_IMPORT_ID) REFERENCES TBL_DATA_IMPORT(ID) ON DELETE CASCADE`);
      ...
      await conn.query(`ALTER TABLE TBL_DATA_EXPORT_FILE ADD CONSTRAINT \`fk_tbl_data_export_file-1\` FOREIGN KEY IF NOT EXISTS (DATA_EXPORT_ID) REFERENCES TBL_DATA_EXPORT(ID) ON DELETE CASCADE`);
