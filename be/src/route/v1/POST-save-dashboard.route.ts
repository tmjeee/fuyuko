...
const httpAction: any[] = [
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        const serializeFormat: SerializeFormat =  req.body.serializeFormat;

        const serializeFormatInString: string = JSON.stringify(serializeFormat);

        await doInDbConnection(async (conn: Connection) => {
            const q: QueryA = await conn.query('SELECT COUNT(*) FROM TBL_USER_DASHBOARD WHERE USER_ID=:userId', [userId]);
