...
class RuleService {
    ...
    async addOrUpdateRules(viewId: number, rules: Rule[]): Promise<string[]> {
        ...
        for (const rule2 of rule2s) {
            if (rule2.id && rule2.id > 0)  { // update
                const errs: string[] = await doInDbConnection(async (conn: Connection) => {
                    const err: string[] = await this._ruleUpdate(conn, viewId, rule2)
                    return err;
                });
                errors.push(...errs);
            } else { // add
                const errs: string[] = await doInDbConnection(async (conn: Connection) => {
                    const err: string [] = await this._ruleAdd(conn, viewId, rule2)
                    return err;
                });
                errors.push(...errs);
            }
    ...
    private async _ruleAdd(conn: Connection, viewId: number, rule2: Rule2) {
        const errors: string[] = [];
    ...
    private async _ruleUpdate(conn: Connection, viewId: number, rule2: Rule2) {
        const errors: string[] = [];
        await doInDbConnection(async (conn: Connection) => {
    ...
    // === updateRuleStatus ===
    // ========================
    async updateRuleStatus(ruleId: number, status: Status): Promise<boolean> {
    ...
    // === getRules ===
    // ================
    async getRules(viewId: number): Promise<Rule[]> {
        const rule2s: Rule2[] = await this.getRule2s(viewId);
    ...
    private async getRule2s(viewId: number): Promise<Rule2[]> {
    ...
    // === getRule ===
    // ================
    async getRule(viewId: number, ruleId: number): Promise<Rule> {
        const rule2: Rule2 = await this.getRule2(viewId, ruleId);
    ...
    private async getRule2(viewId: number, ruleId: number): Promise<Rule2> {
        return await doInDbConnection(async (conn: Connection) => {
            const q: QueryA = await conn.query(SQL_2, [viewId, ruleId]);
            const rs: Rule2[] = this.p(q);
            if (rs.length > 0) {
                return rs[0];
            }
            return null;
