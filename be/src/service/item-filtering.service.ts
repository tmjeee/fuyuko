...
export const getItemWithFiltering = async (viewId: number,
                                           parentItemId: number,
...
export const getItem2WithFiltering = async (
                                 viewId: number,
                                 parentItemId: number,
                                 whenClauses: ItemValueOperatorAndAttribute[]):
                                 Promise<Item2WithFilteringResult> => {
    return await doInDbConnection(async (conn: Connection) => {
        return _getItem2WithFiltering(conn, viewId, parentItemId, whenClauses);
    });
}

export const _getItem2WithFiltering = async (
        conn: Connection,
        viewId: number,
        parentItemId: number,
        whenClauses: ItemValueOperatorAndAttribute[]):
