...
class AuthService {
     ...
     * ====================================
     */
    async isValidForgottenPasswordCode(code: string): Promise<boolean> {
     ...
     * ==============================
     */
   async resetForgottenPassword(code: string, password: string): Promise<string[]> {
     ...
     * ======================
     */
   async forgotPassword (o: {username?: string, email?: string}): Promise<string[]> {
        return await doInDbConnection(async (conn: Connection) => {
            const errors: string[] = []
            if (!o.username && !o.email) {
                ...
                if (q.length > 0) { // found
                    await this.registerForgottenPassword({
                        userId: q[0].ID,
                        email: q[0].EMAIL,
   ...
   async registerForgottenPassword (i: {userId: number, email: string, username: string, firstName: string, lastName: string}): Promise<string[]> {
       return await doInDbConnection(async (conn: Connection) => {
     ...
     * =============
     */
    async login (usrname: string, password: string): Promise<LoginResult> {
